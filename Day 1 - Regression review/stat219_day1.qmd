...
1. Pull the `vt_temps.csv` file into R, create new variables denoting year and month, and create a new object called `vt_jan_temps` that contains only measurements from January.
...
```
   ...
   :::{.callout-warning icon=false appearance="simple"}
   Potentially a moderate, positive trend.
   :::
...
```
   ...
   :::{.callout-warning icon=false appearance="simple"}
   For each additional year, there is an estimated `r round(coef(jan_lm)[2], 5)` centigrade increase in the mean average temperature. There is moderate evidence (p-value of `r round(summary(jan_lm)$coefficients[2,4], 3)`) of a linear association between average temperature and year.
   :::

4. Plot the estimated regression model to the temperature time series. Does it appear to be a good fit to the data?
...
6. Using the (bad) linear regression model, predict the average January temperature for the years 2014 to 2024. Comment on the quality of the prediction. 

```{r, indent = "   ", echo = T, warning = F, message = F}
predict_tbl <- tibble(
  year = 2014:2024,
  AverageTemperature = predict(jan_lm, newdata = tibble(year = 2014:2024))
)
