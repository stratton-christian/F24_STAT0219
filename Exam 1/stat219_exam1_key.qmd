...
```{r, indent = "    ", message = F, warning = F, fig.dim = c(6, 4), eval = F, echo = F}
# one option is to get SSE and sum of interval widths
# fun little loop 
hw.grid <- expand.grid(seq(.1, .9, .1), seq(.1, .9, .1), seq(.1, .9, .1))

loss <- matrix(NA, nrow = nrow(hw.grid), ncol = 2)
pb <- txtProgressBar(min = 0, max = nrow(hw.grid), style = 3, width = 50, char = "=")  
for(i in 1:nrow(hw.grid)){
  tmp <- HoltWinters(
    utah_train_ts,
    alpha = hw.grid[i,1],
    beta = hw.grid[i,2],
  ...
  tmp_pred <- predict(tmp, n.ahead = 60, prediction.interval = T)
  
  loss[i,] <- c(
    sum((utah_test_ts - tmp_pred[,1])^2),
    sum(tmp_pred[,2] - tmp_pred[,3])
  )
  setTxtProgressBar(pb, i)
}
close(pb)

# best model from the loop: c(.1, .1, .1)
```
...
fitted <- ar_fit$x.mean + ar_fit$ar * (utah_train_ts[1:(length(utah_train_ts)-1)] - ar_fit$x.mean)
...
```{r, indent = "    ", message = F, warning = F, fig.dim = c(6, 7)}
res <- resid(sim_reg)
par(mfrow = c(2, 1))
acf(res)
pacf(res)
...
13. [2 pt] Generate two more years ($t = 289, 290, \dots, 312$) from the mean function defined in question 5. Create a new series, `full_ts`, that combines the mean function from $t=1$ to $t=312$ with the `full_error_ts` series, and plot that series. 
    ...
    **Note** that `full_ts` represents the time series between $t=1, \dots, 312$, but we are supposing that we only observed between $t=1, \dots, 288$. The purpose of this part of the question is to generate two more years so that we have something to forecast. 
...
```{r, indent = "    ", message = F, warning = F, fig.dim = c(6, 4), eval = T}
